    ...
    def lookup(comp: dict) -> dict:
        ...
        for ext_match_col in comp["inputs"]["input"]["inputColumns"]["inputColumn"]["properties"]["property"]:
            try: 
                ...
                pass
        mapping = pd.concat([mapping, pd.DataFrame({"Column_lookup": [column], "Column_name": [match_col]})], ignore_index=True)
        ...
        for outputs in output_poss:
            if outputs["name"] == "Lookup Match Output":
                ...
                columns_match = pd.concat([columns_match, pd.DataFrame({"Column_lookup": [lookup_col], "Column_name": [output_col]})], ignore_index=True)
