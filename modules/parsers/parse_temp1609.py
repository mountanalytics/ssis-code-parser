...
def executesql_parser(control_flow, nodes, lineages, variable_tables, node_name):
    ...
    if 'select' in sql_statement.lower():
        ...
        columns = [i.this for i in schema.find_all(exp.Expression)][2:]
        print(columns)

        values = [i for i in insert.find_all(exp.Values)][0]

        transformations = [i.this for i in values.find_all(exp.Expression)]
...
if __name__ == '__main__':
    ...
    a = parse_one("INSERT INTO SSIS_Logger (ExecutionStartTime, Remark, Affected_rows) VALUES (GETDATE(), 'Deleted rows', 0)")
    
    
    
    a.args["this"].args["this"].args["this"].args["this"]
    
    for i in a.args["this"].args["expressions"]:
        print(i.args["this"])
        
    for i in a.args["expression"].args["expressions"][0].args["expressions"]:
        print(i.args["this"])    
