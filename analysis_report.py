...
def load_save_execute(lineages: pd.DataFrame, nodes: pd.DataFrame, dict_blocks: dict):
...
lineages = pd.read_csv('output-data/lineages/lineages.csv')
nodes = pd.read_csv('output-data/nodes.csv')
with open('output-data/dict_blocks_control.json', 'r') as file:
    dict_blocks = json.load(file)
load_save_execute(lineages, nodes, dict_blocks)
